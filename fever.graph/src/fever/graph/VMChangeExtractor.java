...
public class VMChangeExtractor
{
	...
	public List<Node> addVMChangesToCommit(List<PartialFMEvolution> fm_changes, Node commit_node) throws Exception
	{
		...
		for(PartialFMEvolution fm: fm_changes)
		{
			for(Feature f : fm.added_features)
			{
				...
				Node n = createFeatureNode(commit_node, null,f,"ADDED", file_nodes);
				nodes.add(n);
			}
			...
			for(Feature f : fm.removed_features)
			{
				...
				Node n = createFeatureNode(commit_node, f,null, "REMOVED", file_nodes);
				nodes.add(n);
			}
			for(Feature f : fm.modified_features.keySet())
			{
				List<Node> file_nodes = getFileNodes(modified.get(f),commit_node);
				Node n = createFeatureNode(commit_node, f,fm.modified_features.get(f), "MODIFIED", file_nodes);
				nodes.add(n);
			}
		}
	...
	private Node createFeatureNode(Node commit_node, Feature f_old, Feature f_new, String op, List<Node> file_edits) throws Exception
	{
		...
		if(f_old == null)
		{
			boolean visible =  FeatureUtils.isFeatureVisible(f_new);
			n.setProperty(FeatureEditNode._name, f_new.getName());
			n.setProperty(FeatureEditNode._type, f_new.getType().getLiteral());
			//n.setProperty(FeatureEditNode._optionality,FeatureUtils.isFeatureOptional(f_new) ?  "optional" : "mandatory");
			n.setProperty(FeatureEditNode._visibility, visible? "visible" : "internal");
		}
		else
		{
			boolean visible =  FeatureUtils.isFeatureVisible(f_old);
			n.setProperty(FeatureEditNode._name, f_old.getName());
			n.setProperty(FeatureEditNode._type, f_old.getType().getLiteral());
			//n.setProperty(FeatureEditNode._optionality,FeatureUtils.isFeatureOptional(f_old) ? "optional" : "mandatory");
			n.setProperty(FeatureEditNode._visibility,visible? "visible" : "internal");
		}
	...
	private Node buildFeatureDescNode(Feature f) throws Exception
	{
		...
		//n.setProperty(FeatureEditNode._optionality,FeatureUtils.isFeatureOptional(f) ? "optional" : "mandatory");
		n.setProperty(FeatureEditNode._visibility,FeatureUtils.isFeatureVisible(f) ? "visible" : "internal");
		
		if(f.getPromptCondition() != null)
			n.setProperty(FeatureEditNode._visibility_condition,f.getPromptCondition());
		else
			n.setProperty(FeatureEditNode._visibility_condition,"");
		...
		{
			if(s.getCondition()!=null && s.getCondition().length() > 1)
				txt += ("[ "	+ s.getTarget() + " if " + s.getCondition()+ " ] \n") ;
			else
				txt += ("[ "	+ s.getTarget() + " ] \n") ;
		}

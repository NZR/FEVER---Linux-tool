...
public class FileChangeExtractor
{
	...
	public List<Node> addFileChangesToCommit(List<FileChange> files, Node commit_node) throws Exception
	{
		...
		for(FileChange fc : files)
		{
			...
			Node file_edit = createNodeForFileChange(commit_node, chg, name) ;
	 ...
	 * 
	 */
	private Node createNodeForFileChange(Node commit_node, ChangeType chg, String name)
	        throws Exception
	{
		Node n  = DBConnection.getService().createNode();
		...
		if(ParsingUtils.isVariabilityFile(name))
			n.setProperty(ArtefactEditNode._type,"vm");
		else if (ParsingUtils.isBuildFile(name))
			n.setProperty(ArtefactEditNode._type,"build");
		else if (ParsingUtils.isSourceFile(name))
			n.setProperty(ArtefactEditNode._type,"source");
		else if (ParsingUtils.isDataFile(name))
			n.setProperty(ArtefactEditNode._type,"data");
		else if (ParsingUtils.isBinaryFile(name))
			n.setProperty(ArtefactEditNode._type,"binary");

...
public class DiffBuilder
{
	...
	public void buildCompositeCommits() throws IOException
	{
		...
		for (int idx = 0; idx < _commits.size(); idx++)
		{
			...
			{
				ArtefactDiff cd = new ArtefactDiff(d, commit);
				...
				_diffs.add(cd);
				diffTexts.reset(); 	//reset the text here, otherwise they get appended, and the diff'
				 					//we extract info from is getting larger with every edited file.
			}
			diffTexts.close();
		}
		revWalk.release();
	...
	private void extractCodeEdits(ByteArrayOutputStream diffTexts, DiffFormatter df, DiffEntry d, ArtefactDiff cd) throws IOException
	{
		df.format(d);
	...
	public void addCodeEditEntriesForDiffText(ArtefactDiff cd, String diff_txt)
	{
	...
	public List<ArtefactDiff> getDiffs()
	{
		return _diffs;
	}
}

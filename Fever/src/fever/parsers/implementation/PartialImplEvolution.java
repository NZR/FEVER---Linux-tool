...
public class PartialImplEvolution {
	...
	public PartialImplEvolution(ImplementationModel old_model , ImplementationModel new_model,String file, ArtefactDiff diff)
	{
		_old= old_model;
		_file_name = file;
		_new = new_model;
	...
	private void assignCodeEditToModdedBlocks()
	{
		for(ConditionalBlock cb : modded_code_blocks)
		{
			for(CodeEdit ce :this._d.changed_lines )
			{
				if (CodeUtils.EditInsideBlock(cb, ce) )
				{
	...
	{		
		for(ConditionalBlock added : added_blocks.keySet())
		{
			...
			for(CodeEdit ce : this._d.changed_lines)
			{
				...
				if(CodeUtils.codeInNewBlockFullyAddedByEdit(added,ce))
				{
				...
				else if (CodeUtils.codeInNewBlockEditedByEdit(added, ce))
				{
	...
	private void extractRemovedBlockEdits()
	{
		for(ConditionalBlock removed : removed_blocks.keySet())
		{
			...
			for(CodeEdit ce : this._d.changed_lines)
			{
				...
				if(CodeUtils.codeInRemovedBlockFullyRemovedByEdit(removed,ce))
				{
				...
				else if (CodeUtils.codeInRemovedBlockEditedByEdit(removed, ce))
				{
	...
	private void cleanBlocks(List<ConditionalBlock> block_list,List<CodeEdit> code_edits)
	{
		...
		for(ConditionalBlock b : block_list)
		{
			...
			for(CodeEdit ce : code_edits)
			{
				...
				{
					if(CodeUtils.touchesIfDefs(ce.getDiff()))
					{
						potentially_ok = true;
					}
				}

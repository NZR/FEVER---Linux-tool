...
public class FeatureOrientedChangeExtractor {
	...
	{
		List<String> fs = FeatureUtils.getFeatureNames(c);
		if(fs.isEmpty())
		{
		...
		for ( PartialMappingEvolution b : s.build_changes)
		{
			for(BuildChange bc : b._symbol_changes)
			{
				...
				for(BuildTargetChange btc : bc.targets)
				{
					...
					if(mc.m_target_type != CompilationTargetType.CC_FLAG )//updated for data/binary docs. was compilation unit or folder.
					{
